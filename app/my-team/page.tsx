...
export default function MyTeamPage() {
  ...
  const pending = appData.applications.filter((p) => p.status === "pending" && appData.activities.some((a) => a.id === p.activityId && a.creatorId === user.id));
  ...
  return <main className="page">
    ...
    {tab === "applications" && <section className="mt-5 space-y-3">{applications.length ? applications.map(({ p, activity }) => <div key={p.id} className="card flex flex-wrap items-center justify-between gap-3"><div><p className="font-black text-ink">{activity.title}</p><p className="mt-1 text-sm text-slate-500">申请留言：{p.message || "未填写"}</p></div><span className={`tag ${p.status === "accepted" ? "bg-emerald-50 text-emerald-600" : p.status === "rejected" ? "bg-rose-50 text-rose-600" : p.status === "withdrawn" ? "bg-slate-100 text-slate-500" : "bg-amber-50 text-amber-600"}`}>{p.status === "accepted" ? "已通过" : p.status === "rejected" ? "未通过" : p.status === "withdrawn" ? "已撤回" : "等待审核"}</span></div>) : <Empty title="还没有申请记录" description="去发现页看看正在招募的活动吧。" />}</section>}
